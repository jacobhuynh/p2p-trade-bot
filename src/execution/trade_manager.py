...
class PaperBroker:
    """
    Immediate-settlement paper broker for backtesting.

    Each call to paper_execute() opens and settles the position instantly
    using the known market result — no open positions are ever held.

    Files written (fresh on each instantiation, not appended):
      backtest_trades.csv   — per-trade log with cost, payout, pnl
      backtest_equity.csv   — cash after every settled trade
      backtest_book.json    — final summary (written by summary())

    Parameters
    ----------
    bankroll : float
        Starting cash in dollars (default 1000.0).
    stake : float
        Fixed dollar amount risked per trade (default 10.0).
        contracts = max(1, floor(stake / cost_per_contract))
    data_dir : str, optional
        Output directory; defaults to PAPER_DATA_DIR env var or "data/paper".
    """

    _TRADES_COLS = [
        "trade_n", "timestamp", "market_ticker", "action", "side",
        "yes_price", "entry_price_cents", "qty", "cost",
        "result", "payout", "pnl", "cash", "note",
    ]
    _EQUITY_COLS = ["trade_n", "timestamp", "cash"]
    ...
    def __init__(
        ...
        self.trades_path = self.data_dir / "backtest_trades.csv"
        self.equity_path = self.data_dir / "backtest_equity.csv"
        self.book_path   = self.data_dir / "backtest_book.json"

        # In-memory state
        self._trade_n     = 0
        self._wins        = 0
        self._pnl_total   = 0.0
        self._equity_curve = [bankroll]   # for max-drawdown calculation

        # Write fresh headers (overwrite any previous backtest run)
        self._init_files()
    ...
    def _init_files(self) -> None:
        with open(self.trades_path, "w", newline="") as f:
            csv.DictWriter(f, fieldnames=self._TRADES_COLS).writeheader()
        with open(self.equity_path, "w", newline="") as f:
            csv.DictWriter(f, fieldnames=self._EQUITY_COLS).writeheader()

    # ── Core execution ────────────────────────────────────────────────────────

    def paper_execute(self, decision: dict, market_result: str) -> dict:
        """
        Immediately settle a trade using the known market result.

        Parameters
        ----------
        decision      : dict from LeadAnalyst.analyze_signal()
        market_result : "yes" or "no" — the known final outcome

        Returns
        -------
        dict with keys: status, ticker, side, qty, cost, payout, pnl, cash, won
        """
        ...
        if yes_price is None:
            return {"status": "SKIPPED", "reason": "No price in decision."}
        ...
        if cost_per_contract <= 0:
            return {"status": "SKIPPED", "reason": f"Invalid price {entry_price_cents}c."}

        # ── Sizing ───────────────────────────────────────────────────────────
        qty  = max(1, int(self.stake / cost_per_contract))
        cost = round(qty * cost_per_contract, 4)

        # ── Settlement ───────────────────────────────────────────────────────
        won    = (side == "YES" and market_result == "yes") or \
                 (side == "NO"  and market_result == "no")
        payout = round(qty * 1.0, 4) if won else 0.0
        pnl    = round(payout - cost, 4)

        self.cash        = round(self.cash + pnl, 4)
        self._pnl_total  = round(self._pnl_total + pnl, 4)
        self._trade_n   += 1
        if won:
            self._wins += 1
        self._equity_curve.append(self.cash)

        # ── Logging ──────────────────────────────────────────────────────────
        ts = datetime.now(timezone.utc).isoformat()

        with open(self.trades_path, "a", newline="") as f:
            csv.DictWriter(f, fieldnames=self._TRADES_COLS).writerow({
                "trade_n":           self._trade_n,
                "timestamp":         ts,
                "market_ticker":     ticker,
                "action":            action,
                "side":              side,
                ...
                "pnl":               pnl,
                "cash":              self.cash,
                "note":              "won" if won else "lost",
            })

        with open(self.equity_path, "a", newline="") as f:
            csv.DictWriter(f, fieldnames=self._EQUITY_COLS).writerow({
                "trade_n":  self._trade_n,
                "timestamp": ts,
                "cash":      self.cash,
            })
    ...
    def max_drawdown(self) -> float:
        """Maximum peak-to-trough decline in cash across the equity curve."""
    ...
    def summary(self) -> dict:
        """Return and persist the final backtest summary to backtest_book.json."""
        n        = self._trade_n
        win_rate = round(self._wins / n, 4) if n > 0 else 0.0
        roi      = round(self._pnl_total / self.bankroll, 4) if self.bankroll > 0 else 0.0

        result = {
            "bankroll":     self.bankroll,
            "final_cash":   self.cash,
            "total_pnl":    self._pnl_total,
            "n_trades":     n,
            "n_wins":       self._wins,
            "win_rate":     win_rate,
            "roi":          roi,
